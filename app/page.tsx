...
export default function Home() {
  return <main>
    ...
    <section id="builds" className="section shell"><div className="sectionHead"><div><div className="eyebrow">SELECTED BUILDS</div><h2>Useful software, not just pretty screens.</h2></div><EventLink className="index" href="/builds" event="build_view">VIEW ALL →</EventLink></div><div className="cards">{builds.map((p,i)=><EventLink className="card" href={`/builds/${p.slug}`} event="build_view" key={p.slug}><div className="preview"><img className="projectPreviewImage" src={previewMap[p.slug]} alt={`${p.name} project preview`} loading="lazy" /><div className="previewShade"/><span className="previewNumber">0{i+1}</span><div className="previewTitle">{p.name}</div><div className="previewStatus">{p.status}</div></div><div className="cardMeta"><span className="status">{p.status}</span><span>BUILD / 0{i+1}</span></div><h3>{p.name}</h3><p>{p.summary}</p><span className="textLink">View case study →</span></EventLink>)}</div></section>
