...
export default function Home() {
  return <main>
    ...
    <section id="top" className="hero shell">
      ...
      <div className="eyebrow heroEyebrow"><span className="liveDot" /> INDEPENDENT SOFTWARE-BUILDING STUDIO <span>• 2026</span></div>
    ...
    <section id="builds" className="section shell">
      ...
      <div className="cards">{builds.map((p, i) => <article className="card" data-tilt data-cursor-view key={p.slug}>
        ...
        <div className="cardMeta"><span className="status">{p.status}</span><span>BUILD / 0{i + 1}</span></div><h3>{p.name}</h3><p>{p.summary}</p><div className="cardBottom"><EventLink className="textLink" href={`/builds/${p.slug}`} event="build_view">Case study →</EventLink>{p.github && <a className="sourceLink" href={p.github} target="_blank" rel="noreferrer">SOURCE ↗</a>}</div>
      </article>)}</div>
    </section>

    <section className="section darkSection ideaFlow"><div className="shell"><div className="eyebrow">02 / FROM IDEA → LIVE WEBSITE</div><div className="flowHeader"><h2>Brief in. <i>Business tool out.</i></h2><p>Every build starts with the business objective, not a template.</p></div><div className="flowGrid"><div className="flowStep"><span>01</span><b>BRIEF</b><strong>Local roofing company</strong><p>Generate more qualified enquiries.</p></div><div className="flowLine"/><div className="flowStep"><span>02</span><b>DESIGN</b><strong>Trust + clarity</strong><p>Services, proof and one obvious next action.</p></div><div className="flowLine"/><div className="flowStep"><span>03</span><b>BUILD</b><strong>Fast, responsive UI</strong><p>Mobile-first layout with focused interactions.</p></div><div className="flowLine"/><div className="flowStep active"><span>04</span><b>DEPLOY</b><strong>LIVE WEBSITE ↗</strong><p>Test, ship, measure and improve.</p></div></div></div></section>
