...
export function ProjectForm() {
  ...
  return <>
    {submitted && <div className="formSuccess" role="status">Project brief prepared. Your email client should open with the details. If it does not, email hello@hariombuilds.com directly.</div>}
    <form className="projectForm" action="mailto:hello@hariombuilds.com" method="post" encType="text/plain" onFocus={() => { if (!started.current) { started.current = true; track('form_start'); } }} onSubmit={() => { track('form_submit'); setSubmitted(true); }}>
